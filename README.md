...
단어마다 1~5의 난이도(`level`)가 매겨져 있고, 현재 직급(랭크)이 낮을수록 쉬운 단어가 압도적으로 많이 나오도록 되어 있습니다. 랭크가 오를수록 점점 어려운 단어 비중이 늘어납니다.
...
이 표는 레벨별 실제 단어 개수와 무관하게 적용되는 목표 출제 비율입니다 (`src/game/spacedRepetition.js`의 `LEVEL_SHARE_BY_RANK`). 단어당 가중치가 아니라 등급별 비율을 먼저 정하고, 그 비율을 해당 레벨의 실제 단어 개수로 나눠서 단어당 가중치를 계산하기 때문에 나중에 단어를 추가/삭제해도 비율이 유지됩니다.
